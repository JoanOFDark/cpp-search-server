...
public:
    ...
    explicit SearchServer(const StringContainer& stop_words)
        ...
        bool check = !all_of(stop_words.begin(), stop_words.end(), [](const string& word) {
                        return IsValidWord(word);
                        });
        if (check) {
            throw invalid_argument("Incorrect symbol"s);
        }
    }
...
private:
    ...
    QueryWord ParseQueryWord(string text) const {
        if (text.empty()) {
            throw invalid_argument("Text must be not empty"s);
        }
        bool is_minus = false;
        if (text[0] == '-') {
            is_minus = true;
        ...
        if (text.empty() || text[0] == '-' || !IsValidWord(text)) {
            throw invalid_argument("Incorrect text"s);
        }
